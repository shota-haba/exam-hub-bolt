/*
  ...
  2. New Tables
    - `exam_likes` - Track user likes for shared exams
      - `user_id` (uuid, references auth.users)
      - `exam_id` (uuid, references exam_sets)
      - `created_at` (timestamp)
  
  3. Security
    - Enable RLS on `exam_likes` table
...
-- Create exam_likes table (renamed from likes)
CREATE TABLE IF NOT EXISTS exam_likes (
